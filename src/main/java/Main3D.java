...
public class Main3D {
    ...
    private void loop() {
        ...
        // Adiciona inimigos do tipo tanque
        ...
        float soloMinX = dados.Constantes.mapa.x;
        float soloMaxX = dados.Constantes.mapa.x + tileSize * (wh - 1);
        float soloMinZ = dados.Constantes.mapa.z;
        float soloMaxZ = dados.Constantes.mapa.z + tileSize * (wh - 1);

        int quantidadeInimigos = 10;
        float minDistPlayer = 3.0f;
        float minDistEntreTanks = 2.0f;
        float raioTank = 0.01f; // pequeno
        ...
        for (int i = 0; i < quantidadeInimigos; ) {
            ...
            // Calcula índice do heightmap
            int ix = Math.round((x - dados.Constantes.mapa.x) / tileSize);
            int iz = Math.round((z - dados.Constantes.mapa.z) / tileSize);

            // Só cria se dentro do heightmap
            if (ix < 0 || iz < 0 || ix >= wh || iz >= wh) continue;

            float altura = dados.Constantes.mapa.model.data[iz][ix] * 0.001f * dados.Constantes.mapa.raio + dados.Constantes.mapa.y;

            // Não muito perto do player
            float dx = x - player.x;
            float dz = z - player.z;
            float distPlayer = (float) Math.sqrt(dx*dx + dz*dz);

            // Não muito perto de outros tanks
            boolean longeDeOutros = true;
            for (Object3D obj : listaObjetos) {
                if (obj instanceof obj.InimigoSolo) {
                    float ddx = x - obj.x;
                    float ddz = z - obj.z;
            ...
            if (distPlayer > minDistPlayer && longeDeOutros) {
                obj.InimigoSolo inimigo = new obj.InimigoSolo(x, altura + 0.01f, z, raioTank);
    ...
    public void gameUptade(long diftime) {
        ...
        for (int i = 0; i < listaObjetos.size(); i++) {
            ...
            // Não remove o player mesmo se estiver "morto"
            if (!obj.vivo && obj != player) {
                listaObjetos.remove(i);
                i--;
            }
        }

        // Verifica colisão entre projéteis e caixas
        for (int i = 0; i < listaObjetos.size(); i++) {
            Object3D obj = listaObjetos.get(i);
            if (obj instanceof obj.Projetil && obj.vivo) {
                obj.Projetil projetil = (obj.Projetil) obj;
                for (int j = 0; j < listaObjetos.size(); j++) {
                    Object3D alvo = listaObjetos.get(j);
                    // Só verifica colisão com caixas (ObjtCene) que não são o player
                    if (alvo instanceof obj.ObjtCene && alvo != player && alvo.vivo) {
                        float dx = projetil.x - alvo.x;
                        float dy = projetil.y - alvo.y;
                        float dz = projetil.z - alvo.z;
                        float dist = (float) Math.sqrt(dx*dx + dy*dy + dz*dz);
                        if (dist < projetil.raio + alvo.raio) {
                            alvo.vivo = false;      // Marca a caixa para remoção
                            projetil.vivo = false;  // Marca o projétil para remoção
                            break; // Um projétil só acerta uma caixa
                        }
                    }
                }
            }
